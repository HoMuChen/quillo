...
export function NewProjectDialog({ locale }: { locale: 'zh-TW' | 'en' }) {
  ...
  if (!open) {
    return <Button variant="primary" onClick={() => setOpen(true)}>{t('new_button')}</Button>
  }

  return (
    <form
      action={handleSubmit}
      className="w-full max-w-md rounded-xl border border-rule bg-bg p-5 shadow-sh-2 space-y-4"
    >
