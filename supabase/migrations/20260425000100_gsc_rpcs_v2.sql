-- gsc_striking_distance: queries with position 5–20 and ≥100 impressions.
-- Uses gsc_query_daily (accurate, not sampled).
...
RETURNS TABLE(
  ...
  WITH agg AS (
    SELECT
      q.query,
      NULL::text AS normalized_page_url,
      SUM(q.clicks)::bigint AS clicks,
      SUM(q.impressions)::bigint AS impressions,
      (SUM(q.position * q.impressions) / NULLIF(SUM(q.impressions), 0))::numeric AS avg_position
    FROM gsc_query_daily q
    WHERE q.project_id = p_project
      AND q.date >= CURRENT_DATE - (p_window_days || ' days')::interval
    GROUP BY q.query
  )
  SELECT
    agg.query,
    agg.normalized_page_url,
    agg.clicks,
    agg.impressions,
    agg.avg_position,
    NULL::uuid AS matching_article_id
  FROM agg
  WHERE agg.avg_position BETWEEN 5 AND 20
    AND agg.impressions >= 100
  ORDER BY agg.impressions DESC
  LIMIT 100;
$$ LANGUAGE sql STABLE;

-- gsc_rising_queries: queries with ≥50% impressions growth.
-- Uses gsc_query_daily (accurate).
...
-- gsc_decaying_pages: pages with ≥30% click drop.
-- Uses gsc_page_daily (accurate).
